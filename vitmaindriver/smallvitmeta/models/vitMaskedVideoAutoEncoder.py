...
class ViTMaskedVideoAutoEncoder(nn.Module):
    def __init__(self, *, img_size, patch_size, frames_depth, num_classes, dim, depth, heads,  mlp_dim_ratio, channels=3, decoder_depth, decoder_heads,decoder_dim,
                 dim_head=16, dropout=0., emb_dropout=0., stochastic_depth=0., tubelet_size=2, is_LSA=False, classToken=False, is_SPT=False,norm_pix_loss=False, in_chans=3):
    ...
    def forward(self, imgs):
        ...
        latent, id_restores = self.forwardEncoder(imgs)
        pred = self.forwardDecoder(latent, id_restores)  # [N, L, p*p*3]
