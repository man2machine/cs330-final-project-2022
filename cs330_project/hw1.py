...
class MANN(nn.Module):
    def __init__(self, num_classes, samples_per_class, hidden_dim,
        ...
        if rnn_type == "lstm":
            ...
            self.layer1 = torch.nn.RNN(num_classes + 784, hidden_dim, batch_first=True,
                num_layers=num_layers)
    ...
    def forward(self, input_images, input_labels):
        ...
        input_labels[:, -1] = 0
        inputs = torch.concat((input_images, input_labels), dim=-1).float()
    ...
    def loss_function(self, preds, labels):
        ...
        #############################
        #### YOUR CODE GOES HERE ####
        return F.cross_entropy(F.softmax(preds[:, -1], dim=2), labels[:, -1])
        #############################
...
def main(config):
    ...
    writer = SummaryWriter(
        f"{config.run_dir}/{config.num_classes}_{config.num_shot}_{config.random_seed}_{config.hidden_dim}"
    )
    ...
    model = MANN(config.num_classes, config.num_shot + 1, config.hidden_dim,
        num_layers=config.num_layers, rnn_type=config.rnn_type)
    model.to(device)
