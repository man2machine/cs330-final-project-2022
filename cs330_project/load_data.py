...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        labels_one_hot = np.tile(np.eye(self.num_classes), (self.num_samples_per_class, 1, 1))
        char_choices = rng.choice(len(self.folders), self.num_classes, replace=False)
        char_folders = [self.folders[i] for i in char_choices]
        
        samples = [] # [N, K, 784]
        for char_folder in char_folders:
            fnames = os.listdir(char_folder)
            shot_choices = rng.choice(len(fnames), self.num_samples_per_class, replace=False)
            fnames = [fnames[i] for i in shot_choices]
            images = np.array([
                self.image_file_to_array(
                    os.path.join(char_folder, n),
                    np.prod(self.dim_input)) for n in fnames])
            samples.append(images)
        samples = np.array(samples, dtype=np.float32).transpose((1, 0, 2)) # [K, N, 784]
        
        query_shuffle_indices = rng.permutation(self.num_classes)
        samples[-1] = samples[-1, query_shuffle_indices]
        labels_one_hot[-1] = labels_one_hot[-1, query_shuffle_indices]
        
        return samples, labels_one_hot
        #############################
