...
class AttentionSpaceTime(nn.Module):
    def __init__(
...
class ViTMaskedVideoAutoEncoder(nn.Module):
    def __init__(self, *, img_size, patch_size, frames_depth, num_classes, dim, depth, heads,  mlp_dim_ratio, channels=3, decoder_depth, decoder_heads, decoder_dim,
        ...
        super().__init__()
        image_height, image_width = make_pair(img_size)
        patch_height, patch_width = make_pair(patch_size)
        self.image_height,  self.image_width = make_pair(img_size)
        self.num_patches = int((image_height // patch_height) *
                               (image_width // patch_width) * (frames_depth/tubelet_size))
        ...
        self.pos_embedding = nn.Parameter(torch.zeros(
            1, self.num_patches + 1, self.dim), requires_grad=False)
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(
            1, self.num_patches + 1, decoder_dim), requires_grad=False)
        ...
        self.decoder_pred = nn.Linear(
            self.decoder_dim, patch_size ** 2 * in_chans * self.tubelet_size, bias=True)  # decoder to patch
        # --------------------------------------------------------------------------
   ...
   #     x = self.decoder(x_full, pos_emd_mask.shape[1])  # [B, N_mask, 3 * 16 * 16]
    ...
    def unpatchify(self, x):
        ...
        imgs = x.reshape(shape=(
            x.shape[0], self.numChannels, self.frames_depth, self.image_height, self.image_width))
        return imgs
    ...
    def patchify(self, imgs):
        ...
        # frames depth we are conisdering tube as one unit from where we want to extract tokens.
        d = imgs.shape[2]
        ...
        x = x.reshape(
            shape=(imgs.shape[0], h * w * frames, p**2 * 3 * self.tubelet_size))
        return x
